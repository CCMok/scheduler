import { tryCatch } from '@/libs/_general/service/try-catch'
import 'server-only'
import { ServiceResponse } from '@/libs/_general/service/response'
import { Message } from '@/libs/_general/service/message'
import prisma from '@/libs/_general/database/database-manager'
import { checkCanAccessPost } from '@/libs/auth/general/access-utils'
import { UpdatePostStatusRequest, updatePostStatusRequestSchema } from './update-post-status-request'

export const updatePostStatus = tryCatch(async (request: UpdatePostStatusRequest): Promise<ServiceResponse> => {
  const parsedRequest = updatePostStatusRequestSchema.parse(request)
  
  const canAccess = await checkCanAccessPost(parsedRequest.id)
  if (!canAccess) return {
    isSuccess: false,
    message: Message.UNAUTHORIZED,
  }

  const originalValue = await getOriginalValue(parsedRequest.id);
  if (originalValue === undefined) return {
    isSuccess: false,
    message: Message.NOT_FOUND.replaceAll('{0}', '職位'),
  }

  if (originalValue === parsedRequest.status) return {
    isSuccess: false,
    message: `狀態${Message.NOT_CHANGE}`,
  }

  await saveEntity(parsedRequest);

  return {
    isSuccess: true,
  }
})

const getOriginalValue = async (id: number): Promise<number | undefined> => {
  const post = await prisma.post.findUnique({
    where: { id },
    select: { status: true },
  })
  return post?.status ?? undefined;
} 

const saveEntity = async (request: UpdatePostStatusRequest): Promise<void> => {
  await prisma.post.update({
    where: { id: request.id },
    data: { status: request.status },
  })
}